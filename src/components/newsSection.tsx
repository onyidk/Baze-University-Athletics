    ...
    <section className="py-12 px-8" id="news">
      ...
      <div className="container mx-auto">
        <h2 className="flex items-center text-4xl font-bold mb-12 pb-4 border-b border-gray-700">
          ...
          <a href="#news" className="ml-auto text-black text-2xl">view all &gt;&gt;</a>
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((item) => (
            <div key={item.id} className="bg-white border border-gray-300 rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
              <Image 
                src={item.image}
                alt={item.title}
                width={400}
